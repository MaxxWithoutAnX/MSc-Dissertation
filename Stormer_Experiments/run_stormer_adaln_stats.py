...
FIELDS = ["lead", "endpoint", "knee", "probe", "floor", "delta", "pct_of_knee",
          "pct_of_floor", "ci_lo_pct_of_floor", "ci_hi_pct_of_floor", "bound_pct_of_floor",
          "within_margin"]
...
def _values(tables, tag, idx, endpoint, agg="mean"):
    """Axis or composite value for `tag` on resample `idx`."""
    if endpoint == "composite":
        return rcs.composite(tables, tag, idx, agg)
    deltas, denoms = tables[endpoint]
    return ps.axis_value(deltas[tag], denoms, idx, agg)


def analyse(store, leads=LEADS, block=2, n_boot=4000, margin_pct=MARGIN_PCT):
    rows = []
    for lead in leads:
        ...
        for endpoint in ("composite", "balance", "conservation"):
            ...
            boot = np.array([
                100.0 * (_values(tables, KNEE, i, endpoint)
                         - _values(tables, PROBE, i, endpoint))
                / max(_values(tables, FLOOR, i, endpoint), 1e-12) for i in idx])
            ...
            rows.append({
                ...
                "ci_lo_pct_of_floor": float(lo), "ci_hi_pct_of_floor": float(hi),
                "bound_pct_of_floor": float(max(abs(lo), abs(hi))),
...
def main(argv=None):
    ...
    rows = analyse(store, block=a.block, n_boot=a.n_boot, margin_pct=a.margin)

    outdir = a.outdir or a.probe_dir
    out = os.path.join(outdir, "adaln_marginal_ci.csv")
    ...
    print(f"{'lead':>5} {'endpoint':>13}{'knee':>9}{'+adaln':>9}"
          f"{'% of floor':>12}{'95% CI':>20}{'bound':>8}  within margin")
    for r in rows:
        print(f"{r['lead']:>4}h {r['endpoint']:>13}{r['knee']:9.4f}{r['probe']:9.4f}"
              f"{r['pct_of_floor']:11.2f}%  [{r['ci_lo_pct_of_floor']:6.2f},"
              f"{r['ci_hi_pct_of_floor']:6.2f}]{r['bound_pct_of_floor']:8.2f}"
              f"  {'YES' if r['within_margin'] else 'no':>13}")

    print(f"removes {h['pct_of_floor']:+.2f}% of the floor's damage, 95% CI "
          f"[{h['ci_lo_pct_of_floor']:+.2f}, {h['ci_hi_pct_of_floor']:+.2f}]%.")
    if h["within_margin"]:
        print(f"\nThe whole interval lies inside +/-{a.margin}%. adaln's contribution is "
              f"BOUNDED BELOW {h['bound_pct_of_floor']:.2f}% of the floor's damage at 95% "
              f"confidence -- MEASURED negligibility, not merely an absent effect. "
              f"Pre-registration clause 2 is confirmed on Stormer as it was on Aurora.")
    else:
        print(f"\nThe interval is NOT contained in +/-{a.margin}%. This run cannot establish "
              f"negligibility; state the bound ({h['bound_pct_of_floor']:.2f}%) and keep the "
              f"correspondence PREDICTED rather than measured.")
