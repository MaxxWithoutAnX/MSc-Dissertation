""" Stormer W8A8 ablation anaysis (one group quantised per ablation)
"""
print('hi', flush=True)
import gc
import os
...
config = Int8DynamicActivationInt8WeightConfig()   # W8A8 (int8 weight + dynamic int8 activation)
...
out_dir = f"ablation_W8A8_medium_fullyear_{len(indices)}i"
...
for group, names in groups.items():
    ...
    del m
    gc.collect()   # cheap insurance against host RSS creep; see aurora_pipeline/ablations_W8A8.py
    torch.cuda.empty_cache()
