...
def _group_sort_key(g):
    m = re.match(r"(enc|dec)(\d+)_(qkv|proj|fc1|fc2|attn|mlp)$", g)
    if m:
        comp_rank = {"attn": 0, "qkv": 0, "proj": 1, "mlp": 2, "fc1": 2, "fc2": 3}
        return (_CLASS_RANK[m.group(1)], int(m.group(2)), comp_rank[m.group(3)])
    return (_CLASS_RANK.get(_group_class(g), 7), 0, 0)
...
def recover_dates(init_keys):
    """Chronological datetimes for the ablation's init keys (int indices into the
    sampled nc's time axis). Needed for the deseasonalised SVR denominator and the
    monthly bootstrap blocks. Returns None if they cannot be recovered."""
