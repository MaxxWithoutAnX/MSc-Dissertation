...
def boot_band(per_date):
    """(mean, lo, hi) over the date axis (95% CI, monthly blocks - the raw series
    carries the seasonal cycle). Cross-run verdicts need the paired tests in
    create_comparison_plots.py instead."""
...
fig.suptitle("Lat-weighted RMSE seasonality (48 inits, 4/month over 2020)")
fig.autofmt_xdate()
fig.tight_layout()
...
for ax, (label, var) in zip(axes.flat, KEY_VARS.items()):
    ...
    ax.plot(lead_times, means, marker='o', color='C3', lw=1.2, label='Aurora')
