"""Noise-floor ensemble member script
"""
import os
import json
...
model.load_checkpoint_local(CKPT)      # local file -> works on SLURM (no internet on nodes)
    ...
    with torch.inference_mode():
        for k, pred in enumerate(rollout(model, make_batch(i), steps=STEPS)):
            if k in LEAD_BY_STEP:
                ...
                if j is None:
                    print(f"  WARN: no truth for init {i} at lead {lead}", flush=True)
                else:
                    m_all = compute_all_metrics(
                        pred_to_tensor(pred), gt_to_tensor(j), None, VARIABLES, lat_eval, lead)
                    # Drop the per-cell bias maps (69 x 720 x 1440 fp32 = 286 MB of the 288 MB
                    # a lead costs), as ablations_W8A8.py and precision_harness.py both do.
                    # Without this a member is 13.8 GB, not the ~110 MB this script's header
                    # claims, and the 16-member array is 222 GB instead of ~1.8 GB.
                    m_all.pop("bias", None)
                    out[i][lead] = m_all
            del pred
