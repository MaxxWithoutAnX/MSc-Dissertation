""" Aurora W8A8 SQ ablation anaysis (one group quantised per ablation)
"""
import torch
...
from torchao.prototype.smoothquant import load_smooth_quant_recipe, SmoothQuantObservedLinear
...
SCORE_DEVICE = device if os.environ.get("SCORE_ON_GPU", "1") == "1" else torch.device("cpu")

def _to_cpu(x):
    """Recursively move tensors in a metrics container to CPU so GPU scalars/spectra from
    compute_all_metrics don't accumulate in VRAM across inits/leads/groups."""
...
out_dir = f"ablation_W8A8_smoothquant_medium_sampled_{YEAR}_{PER_MONTH}pm"
os.makedirs(out_dir, exist_ok=True)

RECIPE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                      f"smoothquant_recipe_sampled_{YEAR}_{PER_MONTH}pm_a0.5_i1_s28.pt")
assert os.path.exists(RECIPE), (
    f"missing smoothquant recipe {RECIPE} - run inference_sampled_W8A8_smoothquant.py "
    f"once (it calibrates and saves the recipe)")
            ...
            for k, pred in enumerate(rollout(m, make_batch(i), steps=STEPS)):
                if k in LEAD_BY_STEP:
                    ...
                    if j is None:
                        ...
                        pred_t = pred_to_tensor(pred).to(SCORE_DEVICE)
                        gt = gt_to_tensor(j).to(SCORE_DEVICE)
                        m_all = compute_all_metrics(pred_t, gt, None, VARIABLES, lat_eval, lead)
                        m_all.pop('bias')                                  # big per-cell field, unused
                        out[i][lead] = _to_cpu(m_all)
                        if ref_preds is not None or keep_preds:
                            wind = pred_t[:, WIND_IDX]                     # [1, 26, H, W]
                            if ref_preds is not None:
                                out[i][lead]["dke_pert"] = _to_cpu(difference_kinetic_energy(
                                    wind, ref_preds[i][lead].to(SCORE_DEVICE), WIND_VARS, lat_eval, lead))
                            if keep_preds:
                                preds[i][lead] = wind.cpu()                # keep fp_preds on CPU (~5 GB)
                        del pred_t, gt
                del pred
...
for group, names in groups.items():
    ...
    insert_smooth_quant_observer_filtered(
        m, lambda mod, fqn, members=members: isinstance(mod, nn.Linear) and fqn in members)
    ...
    del m
    gc.collect()   # cheap insurance against host RSS creep; see ablations_W8A8.py
    torch.cuda.empty_cache()
